...
export class ClasseSectionService {
  ...
  async createOrUpdateSectionsOfClasse(
    sectionClasse: ClasseSection,
  ): Promise<ClasseSection> {
    return await this.classeSectionRepository.save(sectionClasse);
  }
  async findOneSectionofAClasse(
    classe: Classe,
    section: Section,
  ): Promise<ClasseSection> {
  ...
  async getAllClasseSection(): Promise<ClasseSection[]> {
    return this.classeSectionRepository.find();
  }
}
